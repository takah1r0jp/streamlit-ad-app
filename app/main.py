...
def get_step_status(step_num, api_key, image_exists, conditions_valid, code_exists):
    if step_num == 1:
        ...
        return "completed" if api_key and image_exists else "active" if api_key and not image_exists else "pending"
    elif step_num == 3:
        return "completed" if api_key and image_exists and conditions_valid else "active" if api_key and image_exists and not conditions_valid else "pending"
    elif step_num == 4:
        return "completed" if code_exists else "active" if api_key and image_exists and conditions_valid else "pending"
    elif step_num == 5:
...
image_exists = st.session_state.uploaded_image_path and os.path.exists(st.session_state.uploaded_image_path)
...
with col1:
    # ステップ1: APIキー設定
    step1_status = get_step_status(1, api_key, image_exists, conditions_valid, code_exists)
    st.markdown(f"""
    ...
    new_api_key = st.text_input("Anthropic APIキーを入力", type="password", value=api_key, key="api_input")
    ...
    # ステップ2: 画像選択
    step2_status = get_step_status(2, bool(new_api_key), image_exists, conditions_valid, code_exists)
    st.markdown(f"""
    ...
    uploaded_file = st.file_uploader("画像をアップロード（デフォルト画像も利用可能）", type=["png", "jpg", "jpeg"])
...
with col2:
    # ステップ3: 正常品条件設定
    step3_status = get_step_status(3, bool(new_api_key), image_exists, conditions_valid, code_exists)
    st.markdown(f"""
    ...
    with col_add:
        if st.button("➕ 追加", use_container_width=True):
            ...
            st.rerun()
    with col_remove:
        if st.button("➖ 削除", use_container_width=True) and len(st.session_state.normal_conditions) > 1:
...
with col3:
    # ステップ4: プログラム生成
    step4_status = get_step_status(4, bool(new_api_key), image_exists, conditions_valid, code_exists)
    st.markdown(f"""
    ...
    # 物体検出設定（コンパクト）
    st.session_state.box_threshold = st.slider("検出しきい値", 0.1, 0.9, st.session_state.box_threshold, 0.1, help="物体検出の信頼度")
    
    generate_button = st.button("🚀 プログラム生成", type="primary", 
                               disabled=not (new_api_key and conditions_valid), 
                               use_container_width=True)
    
    # ステップ5: プログラム実行
    step5_status = get_step_status(5, bool(new_api_key), image_exists, conditions_valid, code_exists)
    st.markdown(f"""
